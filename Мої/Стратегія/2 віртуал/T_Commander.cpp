...
//---------------------------------------------------------------------------
T_Warrior* T_Commander::SearchWarrior(TPoint p)
{
    int i=0;
    while(i<KWarrior && !(Warriors[i]->XY==p))
    {
        i++;
    }
    if(i>=KWarrior)
...
//---------------------------------------------------------------------------
bool T_Commander::isInCity()
{
    return XY==LinkCity->XY;
}
//---------------------------------------------------------------------------
void T_Commander::Draw(TCanvas *Canvas,float s,TPoint p)
{
    TPoint d(XY.x*s+p.x,XY.y*s+p.y);
    Canvas->Ellipse(d.x-s/2,d.y-s/2,d.x+s/2,d.y+s/2);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
