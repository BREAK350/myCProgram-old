...
//---------------------------------------------------------------------------
bool T_Doctor::isAttack(T_Warrior *w)
{
    if(Active && GetNation()==w->GetNation() && sqrt((XY.x-w->XY.x)*(XY.x-w->XY.x)+(XY.y-w->XY.y)*(XY.y-w->XY.y))<=Character.Radius)
    {
        return true;
    }
    return false;
...
//---------------------------------------------------------------------------
int T_Warrior::GetNation()
{
    return LinkCommander->Nation;
}
//---------------------------------------------------------------------------
void T_Doctor::Attack(T_Warrior *w)
{
    if(isAttack(w))
    {
        w->Character.Life+=Character.Attack;
        Active=false;
    }
}
//---------------------------------------------------------------------------
