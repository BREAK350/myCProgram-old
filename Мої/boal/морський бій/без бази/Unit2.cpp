   ...
   if(activ==2)
    {
     ...
     form->Canvas->Ellipse(dg.X+x-r,dg.Y-y+r,dg.X+x+r,dg.Y-y-r);
     ShowInfo();
    }
   form->Canvas->Pen->Style=psSolid;
   form->Canvas->Ellipse(dg.X+x-2,dg.Y-y+2,dg.X+x+2,dg.Y-y-2);
   form->Canvas->MoveTo(dg.X+x,dg.Y-y);
   form->Canvas->LineTo(dg.X+x+3,dg.Y-y-5);
   form->Canvas->LineTo(dg.X+x+45,dg.Y-y-5);
   form->Canvas->TextOutA(dg.X+x+3,dg.Y-y-18,name);
   form->Canvas->TextOutA(dg.X+x+3,dg.Y-y-4,IntToStr(x)+":"+IntToStr(y));
...
{
 int i,j;
 for(i=0;i<enemy.nHealthy;i++)
   for(j=0;j<nHealthy;j++)
...
void mTCursor::Show(TForm *form)
{
 ...
 form->Canvas->MoveTo(dg.X+x-5,dg.Y-y);
 form->Canvas->LineTo(dg.X+x+5,dg.Y-y);
 form->Canvas->MoveTo(dg.X+x,dg.Y-y+5);
 form->Canvas->LineTo(dg.X+x,dg.Y-y-5);
...
void TArmy::ShowInfo()
{
 ...
 Form1->Label13->Caption="Ж "+IntToStr(life);
 Form1->Label14->Caption="А "+IntToStr(A);
 Form1->Label15->Caption="З "+IntToStr(Z);
 Form1->Label16->Caption="Видимість "+IntToStr(30);
 Form1->Label17->Caption="Рад.дії ракет "+IntToStr(rocket.r);
 Form1->Label18->Caption="К-сть рак. "+IntToStr(rocket.n);
}
//---------------------------------------------------------------------------
void TCommander::ShowInfo()
{
 Form1->Label4->Caption=name;
}
//---------------------------------------------------------------------------
int TCommander::Healthy()
{
 int i;
 Check();
 nHealthy=0;
...
//---------------------------------------------------------------------------
void DataGame::ReadData()
{
 N=StrToInt(Form1->Edit1->Text);
 nz=StrToInt(Form1->Edit2->Text);
 life=StrToInt(Form1->Edit3->Text);
 A=StrToInt(Form1->Edit4->Text);
 Z=StrToInt(Form1->Edit5->Text);
 rv=StrToInt(Form1->Edit6->Text);
 kr=StrToInt(Form1->Edit7->Text);
 rdr=StrToInt(Form1->Edit8->Text);
 kk=StrToInt(Form1->Edit9->Text);
}
//---------------------------------------------------------------------------
int TCommander::Check()
{
 int i,j;
 for(i=0;i<n;i++)
   if(army[i].life<=0)
