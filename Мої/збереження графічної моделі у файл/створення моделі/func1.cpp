...
void TModel::LoadToFile(char*fname)
{
    ...
    point=(Point2D*)malloc(n_point*sizeof(Point2D));
    fread(point,n_point,sizeof(Point2D),f);
    CreateGraf();
    for(i=0,k=0;i<n_point;i++)
    {
        k+=i;
...
void TModel::Show(Point2D XY)
{
    ...
    for(i=1;i<=n_point;i++)
    {
        for(j=1;j<i;j++)
        {
            Form1->Canvas->MoveTo(point[i-1].x+XY.x,XY.y-point[i-1].y);
            if(isLine(i,j)==1)
            {
                Form1->Canvas->LineTo(point[j-1].x+XY.x,XY.y-point[j-1].y);
            }
        }
    }
}
